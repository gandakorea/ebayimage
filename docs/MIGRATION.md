...
현재 공개 GitHub에는 문서와 명령 도구를 우선 보관한다. 앱 소스·승인 이미지는 공개 업로드 승인 대기 상태다. 기존 컴퓨터의 `migration-backup/ebayimage-full-source-265abab.zip`을 새 컴퓨터에 별도로 옮겨 풀면 앱 소스와 승인 이미지도 복원된다. 아래 앱 설치 명령은 이 ZIP을 복원한 뒤 실행한다.
...
`39220-25500_2.png`는 전체 소스 백업 ZIP에 포함된 승인 비교 이미지다. 작업규칙에 언급된 다른 승인 이미지도 보유하고 있으면 별도로 이전한다.
...
앱의 `.openai/hosting.json`은 실행 설정이 참조하는 비밀정보 없는 프로젝트 식별자와 바인딩 설정으로 전체 소스 백업 ZIP에 함께 보관한다. 기존 호스팅 프로젝트를 가리키므로 새 배포를 만들 때에만 별도로 검토한다. 이번 이전 작업에서는 배포하지 않는다.
