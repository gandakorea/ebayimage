...
if (!manifestFile || !imageDirectory) throw new Error('사용법: node scripts/publish-au-package.mjs <manifest.json> <완성본 폴더>');
...
const base = automationEnv.AUTOMATION_PUBLIC_APP_URL ?? 'https://korea-autoparts-listing-work.vercel.app';
const response = await fetch(`${base.replace(/\/$/, '')}/api/automation/au-only`, {
  method: 'POST',
  headers: { authorization: `Bearer ${automationEnv.AUTOMATION_ADMIN_SECRET}`, 'content-type': 'application/json' },
  body: JSON.stringify({ action: 'publish', package: pkg }),
});
const result = await response.text();
if (!response.ok) throw new Error(`호주 등록 실패 (${response.status}): ${result}`);
console.log(result);
