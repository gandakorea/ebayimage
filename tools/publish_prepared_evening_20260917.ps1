param([switch]$Publish,[ValidateSet('US','AU')][string]$Market='US')

$ErrorActionPreference = 'Stop'
$projectRoot = Split-Path $PSScriptRoot -Parent
$date = '2026-09-17'
$packageRoot = Join-Path $projectRoot "작업중\패키지\2026-09-17-evening"
$resultRoot = Join-Path $projectRoot "작업중\등록결과\2026-09-17-evening"
...
function New-UsRequest($Package, $Uploads, [string]$Call) {
    ...
    $sku = "$($Package.partNumber)-US-20260917-$(([string]$Package.itemId).Substring(0,8))"
...
foreach ($package in $packages) {
    $sku = "$($package.partNumber)-AU-20260917-$(([string]$package.itemId).Substring(0,8))"
    $files = Get-Images $package.partNumber
