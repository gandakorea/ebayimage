param([switch]$Publish,[ValidateSet('US','AU')][string]$Market='US',[string]$OnlyPart)

$ErrorActionPreference = 'Stop'
$projectRoot = Split-Path $PSScriptRoot -Parent
$date = '2026-09-19'
$packageRoot = Join-Path $projectRoot "작업중\패키지\2026-09-19"
$resultRoot = Join-Path $projectRoot "작업중\등록결과\2026-09-19"
...
function Specifics-Xml($Specifics) {
    ...
    foreach ($property in $Specifics.PSObject.Properties) {
        ...
        $values = @($property.Value | ForEach-Object { '<Value>'+(Xml ([string]$_))+'</Value>' }) -join ''
        $parts += '<NameValueList><Name>'+(Xml $property.Name)+'</Name>'+$values+'</NameValueList>'
    }
    $parts -join ''
}
...
function New-UsRequest($Package, $Uploads, [string]$Call) {
    ...
    $sku = "$($Package.partNumber)-US-20260919-$(([string]$Package.itemId).Substring(0,8))"
...
function Assert-Public($Item, $Package, [string]$Seller, [string]$Currency, [int]$ImageCount, [int]$CompatibilityCount) {
    ...
    $titleMismatch = [string]$public.Title -ne [string]$Package.title
    $priceMismatch = $false
    if ($Currency -eq 'USD') {
        $actualPrice = [decimal]([string]$public.StartPrice.'#text')
        $expectedPrice = [decimal]([string]$Package.usdPrice)
        $priceMismatch = $actualPrice -ne $expectedPrice
    }
    if ($titleMismatch -or $priceMismatch) { throw "Published title/price verification failed: $($Package.partNumber)" }
...
if($Market-eq'AU'){$auIdentity = Invoke-RestMethod 'https://apiz.ebay.com/commerce/identity/v1/user/' -Headers @{Authorization="Bearer $auToken"}

if ($auIdentity.username -ne 'sihooshop' -or $auIdentity.registrationMarketplaceId -ne 'EBAY_AU') { throw 'AU account mismatch.' }}

$packages = @(Get-ChildItem -LiteralPath $packageRoot -Filter '*.json' -File | Where-Object { $_.BaseName -notmatch '-(au-audit|archive)$' } | Sort-Object Name | ForEach-Object { Get-Content -Raw -LiteralPath $_.FullName | ConvertFrom-Json })
...
foreach ($package in $packages) {
    if($OnlyPart -and $package.partNumber-ne$OnlyPart){continue}
    $sku = "$($package.partNumber)-US-20260919-$(([string]$package.itemId).Substring(0,8))"
    ...
    $results += $result
    Write-Host "$($result.marketplace) $($result.part) $($result.status) $($result.listingId)"
}

Write-Json (Join-Path $resultRoot 'us-batch-result.json') $results
$results | ConvertTo-Json -Depth 12
exit
}
$usDone=@(Get-ChildItem $resultRoot -Filter '*-US.json'|ForEach-Object{Get-Content -Raw $_.FullName|ConvertFrom-Json}|Where-Object{$_.status-eq'ACTIVE'})
if($usDone.Count-ne4){throw 'US batch incomplete; AU blocked'}
...
foreach ($package in $packages) {
    $sku = "$($package.partNumber)-AU-20260919-$(([string]$package.itemId).Substring(0,8))"
    $files = Get-Images $package.partNumber
    ...
    if (-not $listingId) {
        ...
        $aspects['Manufacturer Part Number'] = @($package.manufacturerPartNumber)
        ...
        $inventory = @{availability=@{shipToLocationAvailability=@{quantity=5}};condition='NEW';product=@{title=$package.title;brand='Genuine Hyundai Mobis';mpn=$package.manufacturerPartNumber;imageUrls=@($uploads.url);aspects=$aspects}}
...
Write-Json (Join-Path $resultRoot 'au-batch-result.json') $results
$results | ConvertTo-Json -Depth 12
