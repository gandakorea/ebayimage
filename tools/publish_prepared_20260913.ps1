...
function New-UsRequest($Package, $Uploads, [string]$Call) {
    ...
    $sku = "$($Package.partNumber)-US-20260913-$(([string]$Package.itemId).Substring(0,8))"
...
if ($Publish -and @($results | Where-Object {$_.marketplace -eq 'EBAY_US' -and $_.status -eq 'ACTIVE'}).Count -ne 1) { throw 'US batch did not complete; AU was not started.' }
...
foreach ($package in $packages) {
    $sku = "$($package.partNumber)-AU-20260913-$(([string]$package.itemId).Substring(0,8))"
    $files = Get-Images $package.partNumber
