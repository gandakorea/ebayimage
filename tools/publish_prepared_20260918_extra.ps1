...
$date = '2026-09-18'
$packageRoot = Join-Path $projectRoot "작업중\패키지\2026-09-18-extra"
$resultRoot = Join-Path $projectRoot "작업중\등록결과\2026-09-18-extra"
...
function New-UsRequest($Package, $Uploads, [string]$Call) {
    ...
    $sku = "$($Package.partNumber)-US-20260918-$(([string]$Package.itemId).Substring(0,8))"
...
if ($packages.Count -ne 2) { throw "Expected two packages, got $($packages.Count)" }
...
if($usDone.Count-ne2){throw 'US batch incomplete; AU blocked'}
...
foreach ($package in $packages) {
    $sku = "$($package.partNumber)-AU-20260918-$(([string]$package.itemId).Substring(0,8))"
    $files = Get-Images $package.partNumber
