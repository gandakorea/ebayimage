...
function Assert-Public($Item, $Package, [string]$Seller, [string]$Currency, [int]$ImageCount, [int]$CompatibilityCount) {
    ...
    if (@($public.ItemCompatibilityList.Compatibility).Count -ne $CompatibilityCount) { throw "Published compatibility count verification failed: $($Package.partNumber)" }
...
if ($Publish -and @($results | Where-Object {$_.marketplace -eq 'EBAY_US' -and $_.status -eq 'ACTIVE'}).Count -ne 2) { throw 'US batch did not complete; AU was not started.' }
...
foreach ($package in $packages) {
    $sku = "$($package.partNumber)-AU-20260912-$(([string]$package.itemId).Substring(0,8))"
    $files = Get-Images $package.partNumber
    ...
    if (-not $listingId) {
        ...
        $aspects['Country of Origin'] = @('Korea, Republic of')
        ...
        $compatibilityAudit = Invoke-EbayJson $auToken ($inventoryUrl+'/product_compatibility') 'GET'
        if (@($inventoryAudit.product.imageUrls).Count -ne $files.Count -or @($compatibilityAudit.compatibleProducts).Count -ne @($package.au.compatibility).Count) { throw "AU inventory verification failed: $($package.partNumber)" }
        $offer = @{sku=$sku;marketplaceId='EBAY_AU';format='FIXED_PRICE';availableQuantity=5;categoryId=$package.au.categoryId;merchantLocationKey='sihooshop-korea';listingDescription=$package.descriptionHtml;listingPolicies=@{paymentPolicyId='178186266024';returnPolicyId='110686678024';fulfillmentPolicyId='257425043024'};pricingSummary=@{price=@{value=$aud;currency='AUD'}};storeCategoryNames=@($package.au.storeCategoryNames);listingDuration='GTC';includeCatalogProductDetails=$true}
        if ($existing.Count) {
