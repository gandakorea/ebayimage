...
$token = ((Get-Content -LiteralPath (Join-Path $root '.env.ebay.local') | Where-Object { $_ -match '^EBAY_US_ACCESS_TOKEN=' } | Select-Object -First 1) -replace '^EBAY_US_ACCESS_TOKEN=', '').Trim()
if ([string]::IsNullOrWhiteSpace($token)) { throw 'Missing EBAY_US_ACCESS_TOKEN.' }
$headers = @{
...
try {
 ...
 if ([string]$result.GetItemResponse.Ack -notin @('Success','Warning')) {
  $message = @($result.GetItemResponse.Errors | ForEach-Object { [string]$_.LongMessage }) -join '; '
